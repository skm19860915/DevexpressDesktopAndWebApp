...
{
    public class ServiceBusiness : AdEngine
    {

        public ServiceBusiness(IDbContext aContext) : base (aContext)
        {
        }
        public List<Service> Get(int? aParentID)
        {
            return new ServiceDataAccess(mContext).Get(aParentID);
        }

        public IEnumerable<Service> GetAll()
        {
            return new ServiceDataAccess(mContext).GetAll();
        }

        public object GetAdStream(string aUserId, int aAdID)
        {
            ...
            var lQuery = (from lDBAd in mContext.Ads
                          ...
                          where lDBAd.AdType == Ad.AdTypes.Service
                          select new
                          {
                              AdID = lDBAd.AdID,
                              AdType = lDBAd.AdType,
