...
namespace WebApp.Controllers
{
    ...
    public class PagesController : Controller
    {
        ...
        public PagesController(RepositoryContext context) {
            _context = context;
        }
        ...
        [HttpGet]
        public async Task<IActionResult> ContentsLookup(DataSourceLoadOptions loadOptions) {
            var lookup = from i in _context.Contents
                         orderby i.Header
                         select new {
                             Value = i.Id,
                             Text = i.Header
        ...
        [HttpGet]
        public async Task<IActionResult> PageTypesLookup(DataSourceLoadOptions loadOptions) {
            var lookup = from i in _context.PageTypes
                         orderby i.Name
                         select new {
                             Value = i.Id,
                             Text = i.Name
        ...
        private void PopulateModel(Page model, IDictionary values) {
            ...
            string CONTENT_ID = nameof(Page.ContentID);
            string MAIN_IMAGE_ID = nameof(Page.MainImageID);
            string PUBLISHED = nameof(Page.Published);
            string PUBLISHED_ON = nameof(Page.PublishedOn);
            string AUTHOR_ID = nameof(Page.AuthorID);
            string PAGE_TYPE_ID = nameof(Page.PageTypeId);
            ...
            if(values.Contains(CONTENT_ID)) {
                model.ContentID = values[CONTENT_ID] != null ? Convert.ToInt32(values[CONTENT_ID]) : (int?)null;
            }

            if(values.Contains(MAIN_IMAGE_ID)) {
                model.MainImageID = values[MAIN_IMAGE_ID] != null ? Convert.ToInt32(values[MAIN_IMAGE_ID]) : (int?)null;
            }
            ...
            if(values.Contains(PAGE_TYPE_ID)) {
                model.PageTypeId = Convert.ToInt32(values[PAGE_TYPE_ID]);
            }
        }
